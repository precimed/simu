...
if ~exist('config', 'var')
    ...
    t = 'E:\noLD_100K_1M_merged';                   if exist(t, 'dir'), noLD_100K_1M_merged_path = t; end;
    t = '/work/users/oleksanf/noLD_100K_1M_merged'; if exist(t, 'dir'), noLD_100K_1M_merged_path = t; end;

    % Matlab 1M template
    config.frames.noLD_100K_1M_merged.subj = 100000;
    config.frames.noLD_100K_1M_merged.snps = 1188973;
    config.frames.noLD_100K_1M_merged.reader = @(subjvec, snpvec)merged_frame_reader(subjvec, snpvec, 100000, fullfile(noLD_100K_1M_merged_path, 'all'));
    config.frames.noLD_100K_1M_merged.bim = PlinkRead_bim(fullfile(noLD_100K_1M_merged_path, 'all'));
    config.frames.noLD_100K_1M_merged.bim = rmfield(config.frames.noLD_100K_1M_merged.bim, {'chrvec', 'cMvec', 'bpvec', 'A1vec', 'A2vec'});

    % Matlab 1M reference template (w_hm3)
    config.frames.noLD_100K_1190K_ref.subj = 100000;
    config.frames.noLD_100K_1190K_ref.snps = 1190321;
    config.frames.noLD_100K_1190K_ref.bim  = PlinkRead_bim(fullfile(norstore_path, '1m_ref'), true, '%s %s %f %d %s %s %s %s');
end
...
frame = make_empty_frame (config, 'noLD_100K_1M_merged');
...
templates = {'noLD_100K_1190K_ref'};
template_short_name  = {'1MnoLD'};
